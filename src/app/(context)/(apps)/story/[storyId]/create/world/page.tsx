...
const WorldSetting = () => {
  ...
  const handleGenerateWorld = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setWorld(response.data.data.world_details);
        setWorldId(response.data.data.worldId);
        store({ worldId: response.data.data.worldId });
      } else {
  ...
  const saveWorld = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        console.log("World saved successfully", response.data.data);
      } else {
      ...
      <div className="bg-[#0f172a] min-h-screen text-white p-8">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8 mt-14">
            <h2 className="text-xl font-semibold mb-4">World Setting</h2>
            <div className="p-5 bg-[#16213e] rounded-lg">
              <h3 className="text-white text-lg mb-2">Initial Idea:</h3>
              <textarea
                value={worldIdea}
              ...
              <div className="mt-4 flex justify-between items-center">
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="return_image" className="text-white">
                    Include Return Image
                  </label>
                </div>

                <ActionButton
                  text={isLoading ? "Generating...." : "Generate World"}
                  highlight
          ...
          <div className="mb-8">
            <h2 className="text-lg font-semibold mb-4">Refine the World</h2>
            <div className="p-5 bg-[#16213e] rounded-lg">
              <textarea
                value={feedback}
          ...
          <div className="flex justify-between mt-4">
            <>
              ...
              />
              <ActionButton
                text={isSaveLoading ? "Saving...." : "Save World"}
                onClick={saveWorld}
                highlight
              />
            </>
            <div>
              <Link href={`/story/${storyId}/create/theme`}>
                <OutlineButton text="Prev: Theme" />
              </Link>
              <Link href={`/story/${storyId}/create/character`} className="ml-2">
