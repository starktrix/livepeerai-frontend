...
const VisualDevelopmentComponent = () => {
  ...
  const openEditModal = (episodeKey, sceneKey, actKey) => {
    const actData = plotData[episodeKey][sceneKey][actKey];
    ...
    setModalContent(
      <EditModal
        ...
        setModalContent={setModalContent}
        imageUrl={imageUrl}
        saveTempVisualUrl={saveTempVisualUrl}
  ...
  const openVideoModal = (episodeKey, sceneKey, actKey) => {
    ...
    setModalContent(
      <VideoModal
        ...
        imageUrl={imageUrl}
        setModalContent={setModalContent}
        saveTempVideoUrl={saveTempVideoUrl}
        parseNumberFromText={parseNumberFromText}
        storyId={storyId}
      />
    );
  ...
  return (
    <>
      ...
      <section className="w-full h-full max-w-5xl mx-auto p-10">
        <div className="text-white text-xl mt-10">Visual Development</div>
            ...
            return (
              ...
              >
                <div className="flex justify-between mb-2">
                  ...
                  <button
                    className={`text-2xl bg-[#1a1a2e] rounded-lg w-14 h-14 text-white flex items-center justify-center ${
                      !!tti && !isLoading ? "cursor-wait" : "cursor-pointer"
                    }`}
                    title="Generate Image from story plot"
                    // TODO: refine this later to include dialogue and emotional cues
                    onClick={() =>
                      handleTextToImage(`Description: ${actData.description}`, {
                        episode: selectedEpisode,
                        scene: episodeState[selectedEpisode].scene,
                        act: actKey,
                      })
                    }
                    disabled={!!tti && !isLoading} // Disable other buttons while one is loading
                  >
                    {" "}
                    {isLoading ? "⏳" : "🖼️"}
                  </button>
        ...
        <div className="bg-[#16213e] rounded-lg mt-6 p-4">
          ...
          <div className="flex mt-4">
            ...
            <div className="bg-[#1a1a2e] rounded-lg w-1/2 h-44 ml-4 p-4">
              <div className="text-white text-sm">Transition Speed</div>
              <div className="relative mt-2">
                <div className="h-2 bg-[#2a2a4e] rounded-full w-full" />
                <div className="absolute top-1/2 transform -translate-y-1/2 left-1/2 bg-[#e94560] rounded-full w-4 h-4"></div>
              </div>
              <div className="text-white text-sm mt-4">Audio Options</div>
              <div className="bg-[#2a2a4e] rounded-lg w-full h-8 mt-2"></div>
            </div>
          </div>
        </div>

        {/* Bottom Controls */}
        <div className="bg-[#16213e] rounded-lg mt-6 p-4 flex justify-between">
          <ActionButton text="Publish" highlight />
          <ActionButton text="Save Draft" />
          <ActionButton text="Regenerate" />
          <ActionButton text="Download Video" />
          <Link href={`/story/${storyId}/create/plot`}>
            <SolidButton text="Prev: Plot" />
          </Link>
        </div>
      </section>
    </>
...
export default VisualDevelopmentComponent;
