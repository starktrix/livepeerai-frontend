...
const PlotDevelopmentUI = () => {
  ...
  // Handle generating world based on user input
  const handleGeneratePlot = async () => {
    ...
    try {
      ...
      console.log(response.data);
      if (response.data.success) {
        setPlotData(response.data.data.plot_details);
        setPlotId(response.data.data.plotId);
        store({ plotId: response.data.data.plotId });
      } else {
  ...
  // Handle saving the current world state
  const savePlot = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        console.log("Plot saved successfully", response.data.data);
      } else {
    ...
    <>
      ...
      <section className="w-full h-full max-w-5xl mx-auto p-10 text-white">
        <section className="mt-10">
          <h1 className="text-xl text-white">Plot Development</h1>
          <div className="mt-4 p-5 bg-[#16213e] rounded-lg">
            ...
            <div className="mt-2 p-2 bg-[#1a1a2e] rounded-md">
              <textarea
                ...
                placeholder="Enter your plot idea..."
                rows={4}
                className="text-sm text-[#a4a4a4] w-full bg-inherit "
              />
            </div>
            <div className="mt-4 flex justify-end">
              <ActionButton
                text={isLoading ? "Generating...." : "Generate Plot"}
                highlight
        ...
        <div className="mt-10">
          <h1 className="text-xl text-white">Generated Plot</h1>
        ...
        <div className="mt-8">
            <h2 className="text-lg font-semibold mb-4">Refine the Plot</h2>
        ...
        <div className="flex justify-between mt-4">
          <>
            ...
            />
            <ActionButton
              text={isSaveLoading ? "Saving...." : "Save Plot"}
              onClick={savePlot}
              highlight
            />
          </>
          <div>
            <Link href={`/story/${storyId}/create/character`}>
              <OutlineButton text="Prev: Character" />
            </Link>
            <Link href={`/story/${storyId}/create/visual`} className="ml-2">
